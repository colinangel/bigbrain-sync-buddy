...
let appState = {
  ...
  ronsRadioSelectedGenres: [],
  ronsRadioAvailableGenres: [],
  logs: []
};
...
// Check if an artist matches any of the selected genres
function artistMatchesSelectedGenres(artist, selectedGenres) {
  if (!artist || !artist.genres || !selectedGenres || selectedGenres.length === 0) return false;
  const selectedLower = selectedGenres.map(g => g.toLowerCase());
  return artist.genres.some(genre =>
    selectedLower.some(selected => genre.toLowerCase().includes(selected))
  );
}
...
async function buildRonsRadio() {
  ...
  try {
    ...
    if (selectedGenres.length === 0) {
      addLog('No genres selected - please select genres to include in Ron\'s Radio', 'warn');
      await saveState(); // Save available genres
      return { success: false, error: 'No genres selected. Please select which genres to include.' };
    }

    addLog(`Filtering for genres: ${selectedGenres.join(', ')}`);

    // Filter tracks where ANY artist matches the selected genres
    const matchingTracks = [];
    let filteredOutCount = 0;

    for (const trackData of trackDataList) {
      const hasMatchingArtist = trackData.artistIds.some(artistId => {
        const artist = artistMap.get(artistId);
        return artistMatchesSelectedGenres(artist, selectedGenres);
      });

      if (hasMatchingArtist) {
        matchingTracks.push(trackData.uri);
      } else {
        filteredOutCount++;
      }
    }
    ...
    // Remove duplicates
    const uniqueTrackUris = [...new Set(matchingTracks)];

    addLog(`Found ${uniqueTrackUris.length} matching tracks, filtered out ${filteredOutCount} non-matching tracks`);

    if (uniqueTrackUris.length === 0) {
      addLog('No tracks found matching selected genres', 'warn');
      return { success: false, error: 'No tracks found matching selected genres' };
    }
    ...
    if (ronsRadioPlaylist) {
      ...
      const genreList = selectedGenres.slice(0, 5).join(', ') + (selectedGenres.length > 5 ? '...' : '');
      const description = `${genreList} tracks from Best Of playlists. Auto-generated by Sync Buddy.`;
    ...
    return {
      ...
      stats: {
        ...
        tracksFilteredOut: filteredOutCount,
...
// Scan genres from Best Of playlists
app.get('/rons-radio-genres', async (req, res) => {
  if (!appState.sourceToken) {
  ...
  try {
    ...
    if (bestOfPlaylists.length === 0) {
      return res.json({ success: true, genres: [], message: 'No "Best Of" playlists found' });
    }
        ...
        for (const item of tracks) {
          if (item.track && item.track.artists) {
            item.track.artists.forEach(a => {
              if (a && a.id) allArtistIds.add(a.id);
            });
          }
        }
      } catch (error) {
        logger.error(`Error fetching tracks from "${playlist.name}": ${error.message}`);
      }
    }

    // Fetch all artist details in batches of 50
    const artistIdArray = Array.from(allArtistIds);
    ...
    for (let i = 0; i < artistIdArray.length; i += 50) {
      ...
      try {
        ...
        logger.error(`Error fetching artist batch: ${error.message}`);
      }
    }
    ...
    logger.error('Error scanning genres', { error: error.message });
    res.status(500).json({ success: false, error: error.message });
  }
});

// Save selected genres
app.post('/rons-radio-genres', async (req, res) => {
  try {
    const { genres } = req.body;

    if (!Array.isArray(genres)) {
      return res.status(400).json({ success: false, error: 'genres must be an array' });
    }

    appState.ronsRadioSelectedGenres = genres;
    await saveState();

    addLog(`Ron's Radio genres updated: ${genres.length} genres selected`);
